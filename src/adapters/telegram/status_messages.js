...
export function buildGeminiRetryNoticeText({ retryCount = 0, maxRetries = 0, agentId = "", agentLabel = "" } = {}) {
  ...
  const label = resolveAgentNoticeLabel(agentId, agentLabel);
  return label
    ? `⏳ Gemini 혼잡으로 재시도 중 (${cleanRetry}/${cleanMax})… (${label})`
    : `⏳ Gemini 혼잡으로 재시도 중 (${cleanRetry}/${cleanMax})…`;
}

export function buildGeminiModelSwitchNoticeText({ toModel = "", agentId = "", agentLabel = "" } = {}) {
  const modelText = clip(String(toModel || "auto"), 120);
  const label = resolveAgentNoticeLabel(agentId, agentLabel);
  return label
    ? `🔁 혼잡 회피를 위해 모델을 ${modelText}로 전환했어요. (${label})`
    : `🔁 혼잡 회피를 위해 모델을 ${modelText}로 전환했어요.`;
}

export function buildGeminiGiveUpNoticeText({ reason = "", agentId = "", agentLabel = "" } = {}) {
  const label = resolveAgentNoticeLabel(agentId, agentLabel);

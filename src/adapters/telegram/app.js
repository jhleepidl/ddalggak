...
import { createDefaultRunRoute } from "../../application/orchestrator.js";
import {
...
import { createSingleInstanceLock, createShutdownHandler, createPollingErrorHandler, registerLifecycleSignals } from "./lifecycle.js";
import { createTelegramUploadService, formatByteSize } from "./uploads.js";
import {
  createJobRuntimeState,
  makeCancelledError as makeCancelledErrorDomain,
  isCancelledError as isCancelledErrorDomain,
} from "../../application/job_runtime.js";
import { summarizeRuntimeTeamSnapshotLines } from "../../application/runtime_snapshot_display.js";
import { createRuntimeTeamSnapshot } from "../../application/runtime_metadata.js";
import { markActionsSkipped, wasInterruptedByReplan } from "../../application/run_status_cleanup.js";
import {
  applyRunAuthority,
  buildRunAuthority,
...
} from "../../application/runtime_composer.js";
import { createSupervisorRuntimeLoader } from "../../application/supervisor_runtime_loader.js";
import {
  isExplicitTeamConfigurationIntentMessage,
} from "../../application/team_intent.js";
import { buildExplicitTeamReconfigurationActions } from "../../application/team_config_diff.js";
import {
  verifyConversationMembershipMutation,
  createMembershipConfirmationError,
} from "../../application/membership_confirmation.js";
import {
  resolveConversationMembershipTarget,
  summarizeMembershipTarget,
} from "../../application/membership_target.js";
import {
...
} from "../../application/agent_team_commands.js";
import {
  createAgentProfile,
  updateAgentProfile,
...
let bot = null;
let botUsername = "";
let chatRunManager = null;
let shutdown = async () => {};
let releaseTelegramLock = () => {};
...
function buildTelegramBotOptions() {
  ...
  if (TELEGRAM_FORCE_IPV4) options.request = { family: 4 };
  return options;
}

function createAppChatRunManager() {
...
export async function startTelegramApp({ token = TOKEN } = {}) {
  if (!String(token || "").trim()) {
    throw new Error("Missing TELEGRAM_BOT_TOKEN");
  }

  bot = new TelegramBot(String(token).trim(), buildTelegramBotOptions());
  chatRunManager = createAppChatRunManager();

  const singleInstanceLock = createSingleInstanceLock({
    enabled: TELEGRAM_SINGLE_INSTANCE_LOCK,
    lockFile: LOCK_FILE,
  });
  try {
  ...
  releaseTelegramLock = singleInstanceLock.release;
  shutdown = createShutdownHandler({
    bot,
    releaseLock: releaseTelegramLock,
    processImpl: process,
  });
  ...
  const uploadService = createTelegramUploadService({
    bot,
    maxBytes: TELEGRAM_EFFECTIVE_DOWNLOAD_MAX_BYTES,
    allowedExts: TELEGRAM_UPLOAD_ALLOWED_EXTS,
  ...
  const telegramUi = {
    ...
    requireMentionInGroup: TELEGRAM_REQUIRE_MENTION_IN_GROUP,
  };
  const runtimeOps = {
    memory,
    formatMemorySummary,

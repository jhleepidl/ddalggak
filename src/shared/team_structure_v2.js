...
function normalizeParticipant(raw = {}, index = 0) {
  ...
  return {
    ...
    model_role_resolution: asObject(row.model_role_resolution || row.modelRoleResolution),
    collaboration_lane: asObject(row.collaboration_lane || row.collaborationLane),
...
function buildParticipantsFromAgents(agents = []) {
  return asArray(agents)
    .map((agent, index) => normalizeParticipant({
      ...
      model_role: agent?.model_role || agent?.modelRole,
      model_role_resolution: agent?.model_role_resolution || agent?.modelRoleResolution,
      collaboration_lane: agent?.collaboration_lane || agent?.collaborationLane,
...
export function buildRuntimeExecutionProfileFromStructureV2(raw = {}, {
  ...
  const runtimeParticipants = structure.participants.map((entry) => ({
    ...
    model_role: cleanId(entry.model_role || ''),
    model_role_resolution: asObject(entry.model_role_resolution),
    collaboration_lane: asObject(entry.collaboration_lane),
...
export function deriveTeamConfigFromStructureV2(raw = {}) {
  ...
  const agents = structure.participants
    ...
    .map((entry) => ({
      ...
      model: entry.model || '',
      provider: cleanId(entry.provider || ''),
      provider_spec: asObject(entry.provider_spec),
      model_role: cleanId(entry.model_role || ''),
      model_role_resolution: asObject(entry.model_role_resolution),
      collaboration_lane: asObject(entry.collaboration_lane),
      capabilities: uniqStrings(entry.capabilities || [], { limit: 8 }),

...
function normalizeCandidateCapabilitySlot(raw = {}) {
  ...
  return {
    ...
    ...(() => { const split = splitToolishIds(row.required_tool_ids ?? row.requiredToolIds ?? row.required_tools ?? row.requiredTools ?? []); return { runtime_capabilities_required: normalizeStringList(split.runtimeCapabilities, { max: 24, lower: true }), external_tool_requirements: normalizeStringList(split.externalTools, { max: 24, lower: true }) }; })(),

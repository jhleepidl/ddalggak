...
function sanitizeRun(run = {}) {
  return {
    ...
    outcome: run.outcome || (run.passed === true ? 'passed' : 'failed'),
    quality_eligible: run.quality_eligible !== false,
    execution_error: run.execution_error || null,
    ...
    provider_result: {
      ...
      execution_error: run.provider_result?.execution_error || run.execution_error || null,
    },
...
export function buildHarnessEvaluationSyncPayload(summary = {}) {
  return {
    ...
    execution_error_run_count: Number(summary.execution_error_run_count || 0),
    quality_run_count: Number(summary.quality_run_count || 0),

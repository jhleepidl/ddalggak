...
function tokenize(text = "") {
  return normalizeText(text, { lower: true })
    .split(/[^a-z0-9가-힣._-]+/g)
    .map((row) => row.trim())
    .filter(Boolean);
}
...
function uniqueList(rows = []) {
  ...
  for (const row of asArray(rows)) {
    const value = normalizeText(row, { lower: true });
...
function roleCompatibilityScore(role = "", compatibleRoles = []) {
  const cleanRole = normalizeText(role, { lower: true });
  const compatible = uniqueList(compatibleRoles);
...
export function scoreSkillForTask({
  ...
  const capabilityMatches = keywordMatchCount(goalText, skill.capability_tags || []);
  const titleMatches = keywordMatchCount(goalText, [skill.name, skill.slug, skill.description].filter(Boolean));
...
export class SkillResolver {
  ...
  resolveForRole({
    ...
    const candidates = this._listSkills(roleType);
  ...
  resolveForTeam({
    ...
    for (const role of roles) {
      const roleType = normalizeText(role?.role_type || role?.id || role?.role_label, { lower: true });
      if (!roleType) continue;
      const resolved = this.resolveForRole({
        roleType,
        goal,
        contextHints,
        existingAttachments: role?.attached_skills || [],
      });

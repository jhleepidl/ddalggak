...
const BUILTIN_MOTIFS = [
  ...
  {
    motif_id: 'motif.bounded_watch_loop',
    label: 'Bounded watch/improvement loop',
    source: 'builtin',
    pattern: 'graph',
    role_slots: [
      { role_id: 'operator', purpose: 'Maintain iteration state, approval boundaries, and stop-condition checks', parallelizable: false, required_context_types: ['workflow_contract', 'policy', 'iteration_state'] },
      { role_id: 'researcher', purpose: 'Gather fresh evidence, requirements, and risk constraints before changes', parallelizable: true, required_context_types: ['evidence', 'freshness', 'risk'] },
      { role_id: 'builder', purpose: 'Implement the bounded improvement selected for this iteration', parallelizable: false, required_context_types: ['workspace', 'patch_plan'] },
      { role_id: 'reviewer', purpose: 'Verify implementation quality, risks, tests, and whether another iteration is needed', parallelizable: false, required_context_types: ['artifact', 'tests', 'risk', 'stop_condition'] },
      { role_id: 'synthesizer', purpose: 'Report iteration outcome, blockers, approval requests, and next loop decision', parallelizable: false, required_context_types: ['reviewed_artifact', 'handoff'] },
    ],
    coverage_tags: ['bounded_loop', 'implementation_loop', 'verification', 'risk', 'approval_gate'],
    task_types: ['bounded_loop', 'continuous_improvement'],
    coordination_cost: 4,
    default_weight: 1.42,
    parallelism_hint: 'hybrid',
  },

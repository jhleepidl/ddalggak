...
export function recordLlmTrace({
  ...
  const response = {
    ...
    wall_duration_ms: Number.isFinite(Number(result?.wallDurationMs ?? result?.wall_duration_ms)) ? Number(result?.wallDurationMs ?? result?.wall_duration_ms) : null,
    started_at: clean(result?.startedAt || result?.started_at) || null,
    ended_at: clean(result?.endedAt || result?.ended_at) || null,
    attempt_count: Number.isFinite(Number(result?.attempt_count ?? result?.attemptCount)) ? Number(result?.attempt_count ?? result?.attemptCount) : null,
    transport: clean(result?.transport) || null,
  ...
  const indexEntry = {
    ...
    wall_duration_ms: response.wall_duration_ms,

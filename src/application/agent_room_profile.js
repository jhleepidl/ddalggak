...
export function inferAgentRoomArchetype(goal = '', { explicitRoles = [] } = {}) {
  ...
  const review = /(review|verify|test|검토|리뷰|검증|테스트|품질)/i.test(text);
  const build = /(implement|build|code|webapp|app|구현|개발|코드|웹앱|사이트)/i.test(text);
  const risk = /(risk|approval|approve|finance|stock|주식|금융|리스크|승인|위험|큰\s*변경)/i.test(text);
  const evidence = /(news|price|source|evidence|latest|뉴스|가격|근거|최신|출처)/i.test(text);
  ...
  if (review || build || roles.includes('verifier')) recommended.push('verifier');
  if (risk || evidence || roles.includes('risk_reviewer')) recommended.push('risk_reviewer');
  if (evidence || roles.includes('researcher')) recommended.push('researcher');
  if (recommended.length === 0) recommended.push('planner', 'builder', 'reviewer');

  return {
    archetype: loop ? 'iterative_agent_workspace' : (review ? 'review_gated_workspace' : 'agent_room_workspace'),
    default_workflow: loop ? 'bounded_review_improve_loop' : (review ? 'review_gated_pipeline' : 'task_adaptive'),
    recommended_roles: normalizeRoomAgentRoles([...roles, ...recommended]),
    autonomy_policy: {
      small_safe_changes: 'auto',
      risky_or_large_changes: risk ? 'approval_required' : 'review_required',
...
export function buildAgentRoomProfile({ chatId = '', roomName = '', goal = '', roles = [], workflowContract = null, team = null, source = 'telegram_agents_command' } = {}) {
  ...
  return {
    ...
    name: cleanText(roomName || 'Agent Workspace', { maxLen: 120 }),
    status: 'active',
    source,
...
export function formatAgentRoomProfile(profile = null, { includeHelp = true } = {}) {
  ...
  if (includeHelp) {
    lines.push('', 'Commands:', '- /agents suggest <goal>', '- /agents use planner,builder,reviewer', '- /task loop <goal>', '- /review');
  }
  return lines.join('\n');
}
...
export function buildAgentRoomSuggestionMessage({ goal = '', profile = null } = {}) {
  ...
  return [
    ...
    '',
    '적용하려면:',
    `/agents use ${asArray(row.default_agents).join(',') || 'planner,builder,reviewer'}`,
    '',
    '반복 작업까지 바로 시작하려면:',

...
const BENCHMARK_TEMPLATES = {
  deep_research_trio: {
    ...
    seed: (() => {
      ...
      seed.agents = [
        { agent_id: 'deep_research_scout', name: 'Deep Research Scout', role: 'researcher', provider: 'gemini', model: 'gemini-2.5-pro', purpose: 'Expand the search space, cluster sources, and maintain an evidence ledger before synthesis.', required_tool_ids: ['web'], optional_tool_ids: ['read_only_fs'], recommended_tool_ids: ['web'], attached_skill_ids: ['skill.deep_research_workflow.v1', 'skill.claim_evidence_audit.v1'] },
        { agent_id: 'evidence_gate_reviewer', name: 'Evidence Gate Reviewer', role: 'reviewer', provider: 'openai', model: 'gpt-5.4', purpose: 'Challenge unsupported claims, collapse duplicates, and mark unresolved contradictions.', required_tool_ids: ['web'], optional_tool_ids: ['read_only_fs'], recommended_tool_ids: ['web'], attached_skill_ids: ['skill.claim_evidence_audit.v1', 'skill.deep_research_workflow.v1'] },
        { agent_id: 'brief_synthesizer', name: 'Brief Synthesizer', role: 'synthesizer', provider: 'openai', model: 'gpt-5.4', purpose: 'Produce the final brief, open questions, and next actions in operator-friendly form.', optional_tool_ids: ['read_only_fs'], recommended_tool_ids: ['telegram'], attached_skill_ids: ['skill.telegram_briefing.v1', 'skill.deep_research_workflow.v1'] },
      ];
  ...
  repo_delivery_loop: {
    ...
    seed: (() => {
      ...
      seed.agents = [
        { agent_id: 'repo_scout', name: 'Repo Scout', role: 'researcher', provider: 'gemini', model: 'gemini-2.5-pro', purpose: 'Map the codebase, locate affected files, and prepare a bounded implementation brief.', required_tool_ids: ['read_only_fs'], optional_tool_ids: ['web'], attached_skill_ids: ['skill.run_trace_debugging.v1', 'skill.context_selection_policy.v1'] },
        { agent_id: 'client_companion_builder', name: 'Client Companion Builder', role: 'builder', provider: 'codex', model: 'gpt-5-codex', purpose: 'Implement the scoped patch, run checks, and keep implementation notes precise.', required_tool_ids: ['workspace_fs'], optional_tool_ids: ['shell'], attached_skill_ids: ['skill.run_trace_debugging.v1'] },
        { agent_id: 'safety_and_quality_reviewer', name: 'Safety And Quality Reviewer', role: 'reviewer', provider: 'openai', model: 'gpt-5.4', purpose: 'Review the patch for correctness, regressions, and delivery risks before finalization.', required_tool_ids: ['read_only_fs'], optional_tool_ids: ['web'], attached_skill_ids: ['skill.claim_evidence_audit.v1', 'skill.run_trace_debugging.v1'] },

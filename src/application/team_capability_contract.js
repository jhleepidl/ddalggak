...
function normalizeAgentToolSets(agent = {}) {
  ...
  if (required.size === 0 && optional.size === 0) {
    for (const toolId of legacyRecommended) optional.add(toolId);
  } else {
    for (const toolId of legacyRecommended) {
      if (!required.has(toolId)) optional.add(toolId);
    }
  }
  return {
    required: uniqueIds([...required]),
    optional: uniqueIds([...optional].filter((toolId) => !required.has(toolId))),
  };
}
...
function buildAgentContract(agent = {}) {
  ...
  const toolSets = normalizeAgentToolSets(agent);
  const required = new Set(toolSets.required);
  const optional = new Set(toolSets.optional);
...
export function buildTeamCapabilityContract({ team = {}, runtime = null } = {}) {
  ...
  const rowToolSets = normalizeAgentToolSets(row);
  const requiredTools = new Set(rowToolSets.required);
  const optionalTools = new Set(rowToolSets.optional);
  for (const requirement of asArray(requirements.tools)) {
    const toolId = cleanId(requirement?.tool_id || requirement?.toolId);
    if (!toolId) continue;
    if (cleanId(requirement?.severity || 'blocking') === 'blocking') requiredTools.add(toolId); else optionalTools.add(toolId);
  }

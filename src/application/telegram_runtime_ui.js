...
export function formatMemorySummary() {
  ...
  const configuredRoleCount = [role.geminiPreview, role.codexPreview, role.chatgptPreview]
    .filter((value) => String(value || '').trim() && !/^\(empty\)$/i.test(String(value || '').trim()))
    .length;
  return [
    "🧠 Memory",
    `mode=${MEMORY_MODE} effective=${memoryModeWithFallback()}`,
    ...(gocInitError ? [`goc_error=${gocInitError}`] : []),
    `file=${s.filePath}`,
    `operator_notes=${s.noteCount}`,
    `recent_lessons=${s.lessonCount}`,
    `role_overrides=${configuredRoleCount}`,
    "",
    "Review/control",
    "- GoC Review Inbox에서 memory proposals, rule candidates, skill/materialization candidates를 승인/편집하세요.",
    "- Telegram은 상태 확인 중심입니다. 명시적 runtime rule만 /rule <지침>으로 추가합니다.",
    "",
    "Commands",
    "- /memory",
    "- /rule",
    ...
    "",
    "Debug",
    "- /memory debug topology",
    "- /memory debug pressure",
    "- /memory debug evidence",
    "- /memory debug review",
    "- /memory debug materialize-preview",
  ].join("\n");
}

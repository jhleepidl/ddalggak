...
function parseArgs(argv = []) {
  const out = { runs: [], sourceZips: [], evaluations: [], logs: [], repositories: {} };
...
Optional: --evaluation <dir> --log <file> --out <dir> --archive <zip>`); }
async function main() {
  const args = parseArgs(process.argv.slice(2));
  if (args.help || !args.runs.length) return usage();
  const result = await buildContinuityHandoff({
    ...
    sourceZips: args.sourceZips, evaluationDirs: args.evaluations, logFiles: args.logs,

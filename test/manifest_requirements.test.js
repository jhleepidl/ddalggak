...
test('buildManifestRequirements infers missing tools and credentials from capability gaps', () => {
  const requirements = buildManifestRequirements({
    team: {},
    capabilityGaps: [
      { kind: 'missing_capability', agent_name: 'Builder', capability_id: 'filesystem_write', tool_id: 'workspace_fs', detail: 'workspace_fs missing' },
      { kind: 'missing_credential', agent_name: 'Researcher', credential_key: 'OPENAI_API_KEY', detail: 'credential required' },
    ],
  });

  assert.equal(requirements['tools'].some((entry) => entry.tool_id === 'workspace_fs'), true);
  assert.equal(requirements.summary.tool_count >= 1, true);

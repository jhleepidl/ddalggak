...
test("supervisor runtime loader builds standalone runtime from local capabilities", async () => {
  ...
  assert.match(runtime.conversationMembershipWarning, /unknown_explicit_members:ghost/);
...
test("supervisor runtime loader builds goc runtime with catalog, team, tools, and context", async () => {
  ...
  assert.match(runtime.conversationMembershipWarning, /unknown_explicit_members/);
  assert.ok(warnings.some((line) => line.includes("conversation_membership_warning")));
});

test("supervisor runtime loader keeps baseline defaults active when goc explicit membership sync is empty", async () => {
  const loadSupervisorRuntime = createSupervisorRuntimeLoader({
    composeCapabilitiesForRun: () => ({
      authority: {
        mode: "goc",
        ...
        conversationTeamStore: {
          async ensureTeam() {
            return {
              ...
              rows: [],
              warnings: ["team_sync:list_explicit_members:GoC API GET /api/threads/goc-thread/team failed (404)"],
              baseline_agent_ids: ["planner", "coder"],
            };
          },
        },
      },
    ...
    normalizeSupervisorJobConfig: createNormalizeSupervisorJobConfig(),
    pickBaselineConversationCatalogAgents: (rows) => rows.filter((row) => row.id !== "reviewer").map((row) => row.id),
    summarizeJobConfigDebug: (config) => `job=${config.job_id}`,
    summarizeSelectionState: ({ catalog = [], enabled = [] } = {}) => ({
      catalog_ids: catalog.map((row) => row.id),
      enabled_ids: enabled.map((row) => row.id),
  ...
  const runtime = await loadSupervisorRuntime("job_loader_3", {
    includeContext: false,
    includeGlobal: false,
  });

  assert.deepEqual(runtime.baselineDefaultAgentIds, ["planner", "coder"]);
